...
{
    public abstract class DbLongBaseController<T> : ControllerBase where T : LongBaseEntity
    {

        public abstract DbLongBaseAdapter<T> Adapter();
        [Route("add")]
        [HttpPost]
        ...
        public ResponseEntity<string> deleteById(RequestData<string>? record) {
            ...
            long.TryParse(record.data, out long id);
        ...
        [Route("queryById")]
        [HttpPost]
        public virtual ResponseEntity<T> QueryById(RequestData<long>? record)
        {
